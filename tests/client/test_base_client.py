...
def test_client__connection_timeout(environ):
    arguments = {
        "api_url": "http://dynamics.local/",
        "token_url": "http://token.local",
        "client_id": "client_id",
    ...
    client = DynamicsClient(**arguments, connection_timeout=0)
    assert client._oauth_client.timeout.connect == None
    ...
    with environ(**env, DYNAMICS_CONNECTION_TIMEOUT="12"):
        client = DynamicsClient.from_environment()
        assert client._oauth_client.timeout.connect == 12

    with environ(**env, DYNAMICS_CONNECTION_TIMEOUT="0"):
        client = DynamicsClient.from_environment()
        assert client._oauth_client.timeout.connect == None
