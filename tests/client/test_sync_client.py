...
def test_client__get_next_page__nested__dont_paginate_children(dynamics_client):
    dynamics_client.pagesize = 1
    ...
    response = dynamics_client.get(pagination_rules={"pages": 2, "children": {"foo": {"pages": 0}}})
    assert response == DynamicsClientGetResponse(
        count=None,
...
def test_client__ensure_token__token_already_set(dynamics_client):
    ...
    patch_3 = patch("dynamics.client.sync.DynamicsClient.set_token", return_value=token)
...
def test_client__ensure_token__dont_cache_token():
    dynamics_client = MockClient(cache_token=False)
